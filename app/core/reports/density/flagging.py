...
"""
Density report flagging helpers (Issue #246 / #798 Phase 6).

Canonical location for the live rulebook-backed bin flagging used by density
Markdown reports and save_bins. Not a dead shim — prefer importing from here.
"""
...
def _load_and_apply_segment_metadata(
    ...
    if segments_df is not None:
        ...
        raise ValueError(
            "segments_df is required for apply_flagging in v2 pipeline."
        )
...
def _compute_utilization_percentile(result_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        from app.utilization import add_utilization_percentile, ensure_rpm
...
def apply_flagging(
    df: pd.DataFrame,
    config: FlaggingConfig = None,  # DEPRECATED - kept for compatibility
...
def get_flagged_bins(df: pd.DataFrame) -> pd.DataFrame:
...
def summarize_segment_flags(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for seg_id, group in segment_groups:
        ...
        if flagged_count > 0:
            ...
            flagged_copy['severity_rank'] = flagged_copy['flag_severity'].apply(get_severity_rank)
    ...
    return pd.DataFrame(summaries).sort_values('worst_severity', key=lambda x: x.map(get_severity_rank), ascending=False)
...
def get_flagging_statistics(df: pd.DataFrame) -> Dict[str, any]:
    ...
    if flagged_count > 0:
        worst_severity = flagged.sort_values('flag_severity', key=lambda x: x.map(get_severity_rank), ascending=False).iloc[0]['flag_severity']
