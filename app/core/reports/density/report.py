...
"""
Density Markdown report generator (Issue #246 / #798 Phase 6).

Canonical live path for Schema 1.0.0 density reports (Parquet + metrics SSOT).
"""
...
from app.core.reports.density.flagging import (
    FlaggingConfig,
    apply_flagging,
    get_flagged_bins,
...
import app.flagging as ssot_flagging
...
def create_flagging_config(rulebook: Dict[str, Any], bins_df: pd.DataFrame) -> FlaggingConfig:
...
def convert_json_to_segment_summary(
    ...
    Returns:
        DataFrame with columns matching summarize_segment_flags() output
    """
    import logging
    logger = logging.getLogger(__name__)
