...
def _render_metrics_table_v2(md, ctx, schema_name: str, areal_density: float, los_letter: str) -> None:
...
def _render_operational_implications_v2(md, ctx, schema_config: Dict[str, Any],
                                       schema_name: str, los_letter: str) -> None:
...
def _render_definitions_v2(md) -> None:
...
def render_segment_v2(md, ctx, rulebook):
    ...
    _render_metrics_table_v2(md, ctx, schema_name, areal_density, los_letter)
    
    # Render key takeaways
    _render_key_takeaways_v2(md, los_letter)
...
# Phase 3 cleanup: Removed v1-only report format functions (only called by _regenerate_report_with_intelligence which was removed)
# - _generate_new_report_format() - REMOVED (orphaned wrapper, v2 calls generate_new_density_report_issue246() directly)
# - _generate_legacy_report_format() - REMOVED
# - _generate_tooltips_json() - REMOVED


# Phase 3 cleanup: Removed v1-only helper functions (only called by generate_density_report which was removed)
# - _regenerate_report_with_intelligence() - REMOVED
# - _generate_and_upload_heatmaps() - REMOVED
# - _setup_runflow_output_dir() - REMOVED
# - _finalize_run_metadata() - REMOVED
# - _execute_bin_dataset_generation() - REMOVED
# v2 pipeline calls _generate_bin_dataset_with_retry() directly via app/core/v2/bins.py


# Phase 3 cleanup: Removed generate_density_report() - v1 API only, not used by v2 E2E tests
# def generate_density_report(...) - REMOVED (163 lines)
# This function was only called by /api/density-report endpoint (lazy import in main.py)
# v2 pipeline uses generate_new_density_report_issue246() directly via app/core/v2/reports.py
...
# Phase 3 cleanup: Removed generate_simple_density_report() - v1 API only, not used by v2 E2E tests
