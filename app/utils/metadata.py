...
def detect_runtime_environment() -> Literal["local_docker", "cloud_run"]:
    ...
    Returns:
        "cloud_run" if running in GCP Cloud Run
        "local_docker" if running in local Docker container
...
def detect_storage_target() -> Literal["filesystem", "gcs"]:
    """
    Detect the storage target based on GCS configuration.
    
    Returns:
        "gcs" if GCS uploads are enabled
        "filesystem" if writing to local filesystem only
    
    Examples:
        >>> # With GCS_UPLOAD=true:
        ...
        'filesystem'
    """
    gcs_upload = os.getenv("GCS_UPLOAD", "true").lower() in {"1", "true", "yes", "on"}
    if gcs_upload:
        return "gcs"
    else:
        return "filesystem"
