...
- Enhanced Flow.csv normalization to handle both letter suffixes (N2a→N2) and numeric suffixes (F2→F1, G2→G1, K2→K1)
...
### 1. PyArrow List Columns → Numpy Arrays
...
1. Strip trailing letters: `N2a` → `N2`
2. Normalize numeric suffixes: `F2` → `F1` (if F1 exists in expected segments)
...
**Key Concepts:**
- **Bins**: Time-space bins (0.1km × 30s) that track runner density
...
2. If passing, run full suite: `make e2e-v2` (~30 min)
3. Check container logs: `docker logs run-density-dev`
4. Inspect outputs: `runflow/{run_id}/{day}/`

**Test Targets:**
- `make test-v2` - Run v2 analysis (generates outputs)
...
**Always:**
- Check both `seg_id` and `segment_id` columns (backward compatibility)
...
**Major Phases:**
- ✅ #494: Plan: Runflow v2 refactor (parent)
- ✅ #495-502: Phases 1-8 (Models, API, Density, Flow, Reports, UI, E2E)
- ✅ #504: Phase 10: Deprecation & Cleanup
- ✅ #535: E2E test failure: bins.parquet missing 'event' column (this session)

**Bug Fixes:**
- ✅ #513: Bug: Density pulls in 5k segments
- ✅ #515: Bug: Saturday Density.md Executive Summary shows incorrect bin and segment counts
- ✅ #508: 10k v2 spans are loaded under unused keys
- ✅ #526: Restore `metadata.json` parity for v2 + add day + event details
- ✅ #530: SAT Locations
