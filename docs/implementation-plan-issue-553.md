...
- [ ] API accepts full request payload per Issue #553 specification
- [ ] All validation rules implemented (fail-fast)
- [ ] `analysis.json` created and used as single source of truth
- [ ] `metadata.json` includes full request/response
- [ ] No hardcoded event names remain (except in tests)
- [ ] No hardcoded start times remain (except in tests)
- [ ] No hardcoded file paths remain (except in tests)
