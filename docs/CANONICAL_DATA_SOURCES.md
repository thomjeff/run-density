...
**Issue #512**: All data must come from these sources or fail with clear errors. No hardcoded defaults.
...
|-----------|------------------|----------|------------|-------|
| **Start Time** | Input JSON (`events` list) | **None** - fail if missing | Required in API request | Must be integer (0-1439 minutes after midnight) |
...
**Source**: `events` array in API request payload
```json
{
  "events": [
...
- Must be integer between 0 and 1439 (minutes after midnight)
- Validated in `app/core/v2/validation.py::validate_start_times()`

**Error Handling**: Returns 400 Bad Request if missing or invalid

**Related**: Issue #512 - No hardcoded start times allowed
