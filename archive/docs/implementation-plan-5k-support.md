...
#### 5a. `app/conversion_audit.py` - `get_segment_events()` ⚠️ **ARCHIVED**

**Status:** ✅ **File moved to archive/** - Not used in runtime

**Analysis:**
- No runtime imports found in codebase
- Legacy audit utility for `segments_new.csv` (file no longer exists, renamed to `segments.csv`)
- Standalone CLI tool, not part of application runtime
- **Action:** File archived - no changes needed
...
#### 9a. Segments GeoJSON Generation ⚠️ **CRITICAL GAP IDENTIFIED - LOCATION CONFIRMED**
...
- Generated by `app/core/artifacts/frontend.py` - `export_ui_artifacts()` function
- **CONFIRMED:** File exists (filtered by .cursorignore, accessible at runtime)
- **PROBLEM:** Current logic hardcodes event list to `["Full", "Half", "10K"]` in two locations

**Critical Issues Found:**
1. `app/density_report.py` lines 2534-2548: Hardcoded conversion only includes Full, Half, 10K
   - Missing `elite` and `open` columns in segments_list conversion
   - Missing `5K_from_km` and `5K_to_km` fields
   - This affects segment coordinate generation for Elite/Open segments

2. `app/core/artifacts/frontend.py` lines 510 and 547: Hardcoded events list
   - **Line 510:** `_create_segment_feature()` function
   - **Line 547:** `_build_segment_feature_properties()` function
   - Both use: `[event for event in ["Full", "Half", "10K"] if dims.get(...) == "y"]`
   - Missing `"Elite"` and `"Open"` in the events list

3. `app/core/artifacts/frontend.py` line 509: length_km calculation
   - Currently: `dims.get("full_length", dims.get("half_length", dims.get("10K_length", 0.0)))`
   - Should also check `5K_length` for Elite/Open segments

**Required Changes:**

1. **Fix `app/density_report.py` `_add_geometries_to_bin_features()` function**
   - **Lines 2534-2548:** Add `elite`, `open` columns AND `5K_from_km`, `5K_to_km` fields:
   ```python
...
2. **Fix `app/core/artifacts/frontend.py` - Events Property (TWO LOCATIONS)**
   - **Line 510:** `_create_segment_feature()` function
   - **Line 547:** `_build_segment_feature_properties()` function
   - **Change:** Add `"Elite"` and `"Open"` to events list:
   ```python
   # BEFORE:
   ...
   "events": [event for event in ["Full", "Half", "10K", "Elite", "Open"] 
              if dims.get(event.lower() if event != "10K" else "10K", "") == "y"],
   ```
   - **Note:** The existing logic already handles lowercase column names correctly (`elite`, `open` in CSV map to `"Elite"`, `"Open"` in events array)

3. **Fix `app/core/artifacts/frontend.py` - length_km Calculation**
   - **Line 509:** Update to also check `5K_length`:
   ```python
   # BEFORE:
   ...
   ```
   - **Note:** Apply same change to line 546 in `_build_segment_feature_properties()`

**Files to Review/Modify:**
- `app/density_report.py` - **CRITICAL:** Fix lines 2534-2548 (add elite/open columns + 5K distance fields)
- `app/core/artifacts/frontend.py` - **CRITICAL:** Fix lines 510 and 547 (events list) + lines 509 and 546 (length_km)
- `app/geo_utils.py` - Verify no additional changes needed (uses different data source)
...
5. ~~Test `get_segment_events()` in `conversion_audit.py`~~ - **ARCHIVED** (not used in runtime)
...
- ~~`app/conversion_audit.py`~~ - **ARCHIVED** (not used in runtime)
...
- [x] ~~Update `app/conversion_audit.py`~~ - **ARCHIVED** (not used in runtime)
...
3. ✅ **Runtime discovery complete** - Artifact generation location confirmed (`app/core/artifacts/frontend.py`)
4. **Create feature branch** - `feature/5k-course-support`
5. **Follow Implementation Checklist above** - Systematic phase-by-phase approach
6. **Test thoroughly** - Especially artifact generation and event detection functions

---

## Runtime Discovery Summary (2025-11-24)

**Artifact Generation Location Confirmed:**
- ✅ File exists: `app/core/artifacts/frontend.py` (filtered by .cursorignore, accessible at runtime)
...
- ✅ Logic already handles lowercase CSV columns (`elite`, `open`) correctly
- ✅ Runtime verification: E2E logs show "5️⃣ Generating segments.geojson..." during execution

**Specific Code Changes Required:**
1. **Lines 510 & 547:** Add `"Elite"` and `"Open"` to events list: `["Full", "Half", "10K", "Elite", "Open"]`
2. **Lines 509 & 546:** Add `5K_length` to length_km fallback chain
3. **Lines 2534-2548 (density_report.py):** Add `elite`, `open` columns + `5K_from_km`, `5K_to_km` fields

---

**Plan Created:** 2025-11-23  
**Last Updated:** 2025-11-24 (Runtime discovery: Artifact generation location confirmed)  
...
1. **Data Loading Pipeline** - ✅ **ADDED Section 5b, 9a**
   ...
   - **Action:** Updated plan includes `app/core/flow/flow.py` event detection fix (conversion_audit.py archived - not used in runtime)
